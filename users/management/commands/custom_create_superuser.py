...
class Command(SuperUserCommand):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.useremail_field = self.UserModel._meta.get_field(
            ...
            if field.many_to_many:
                if (
                    ...
                    raise CommandError(
                        ...
                        "relation through model, which "
                        "is not supported." % field_name
                    )
                else:
                    parser.add_argument(
                        "--%s" % field_name,
    ...
    def handle(self, **options):
        ...
        verbose_email_field_name = self.useremail_field.verbose_name
        try:
            self.UserModel._meta.get_field(PASSWORD_FIELD)
        except exceptions.FieldDoesNotExist:
            pass
        else:
            # If not provided, create the user with an usable password.
            user_data[PASSWORD_FIELD] = None
        try:
            if options["interactive"]:
                # Same as user_data but without many to many fields and with
                # foreign keys as fake model isntances instead of row IDs.
                fake_user_data = {}
                ...
                while email is None:
                    message = self._get_input_message(
                        self.useremail_field, "email@valid.com"
                    )
                    email = self.get_input_data(
                        self.useremail_field, message, "email@valid.com"
                    )
                    if email:
                        error_msg = self._validate_email(
                            email, verbose_email_field_name, database
                        )
                        if error_msg:
                            self.stderr.write(error_msg)
                            email = None
                            continue
                user_data[self.UserModel.USERNAME_FIELD] = email
                fake_user_data[self.UserModel.USERNAME_FIELD] = (
                    self.useremail_field.remote_field.model(email)
                    if self.useremail_field.remote_field
                    else email
                )
                ...
                if username is None:
                    username = os.environ.get(
                        "DJANGO_SUPERUSER_" + 'username'.upper()
                    )
                if username is None:
                    raise CommandError(
                ...
                # Use email from environment variable, if not provided in
                # options
                if email is None:
    ...
    def email_is_unique(self):
        if self.useremail_field.unique:
            return True
        for unique_constraint in self.UserModel._meta.total_unique_constraints:
            if (
                len(unique_constraint.fields) == 1
                and unique_constraint.fields[0] == self.useremail_field.name
            ):
                return True
        return False

    def _validate_email(self, email, verbose_field_name, database):
        ...
        try:
            self.useremail_field.clean(email, None)
        except exceptions.ValidationError as e:
